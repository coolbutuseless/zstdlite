...
unsigned char *read_file(const char *filename, size_t *src_size) {
  ...
  if (fp == NULL) Rf_error("read_file(): Couldn't open file '%s'", filename);
  fseek(fp, 0, SEEK_END);
  ...
  if (buf == NULL) {
    Rf_error("read_file(): Could not allocate memory to read %zu bytes from '%s'", fsize, filename);
  }
  ...
  if (n != fsize) {
    Rf_error("read_file(): fread() only read %zu/%zu bytes", n, fsize);
  }
...
unsigned char *read_partial_file(const char *filename, size_t max_bytes, size_t *src_size) {
  FILE *fp = fopen(filename, "rb");
  if (fp == NULL) Rf_error("read_partial_file(): Couldn't open file '%s'", filename);
  fseek(fp, 0, SEEK_END);
  *src_size = (size_t)ftell(fp);
  ...
  if (buf == NULL) {
    Rf_error("read_partial_file(): Could not allocate memory to read %zu bytes from '%s'", max_bytes, filename);
  }
  ...
  if (n != max_bytes) {
    Rf_error("read_partial_file(): fread() only read %zu/%zu bytes", n, max_bytes);
  }
